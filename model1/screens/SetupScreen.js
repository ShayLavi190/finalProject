...
const SetupScreen = ({ navigation, handleGlobalClick }) => {
  ...
  const [bankBranchNumber, setBankBranchNumber] = useState(
    user.bankBranchNumber ? user.bankBranchNumber : ""
  );
  const [selectedBank, setSelectedBank] = useState(
    user.selectedBank ? user.selectedBank : ""
  );
  const [selectedhealthFund, setSelectedhealthFund] = useState(
    user.selectedhealthFund ? user.selectedhealthFund : ""
  );
  const [healthFundAccountNumber, setHealthFundAccountNumber] = useState(
    user.healthFundAccountNumber ? user.healthFundAccountNumber : ""
  );
  ...
  const validateInputs = () => {
    ...
    if (!healthFundAccountNumber.trim())
      errors.push("מספר חשבון קופת חולים נדרש.");
    ...
    >
      <View style={styles.card}>
        ...
        <Text style={styles.subtitle}>
          כדי שהרובוט המטפל יוכל להפעיל את שירותיו לטובך נצטרך את פרטיך האישיים
          לכך. כלל המידע נשמר בצורה מאובטחת ואינו משותף עם שום גורם חיצוני ללא
          ביצוע שירות יעודי.
        </Text>
        <TextInput
          style={styles.input}
        ...
        <TextInput
          style={styles.input}
          placeholder="   כתובת מגורים במבנה: כתובת ,דירה אם יש, ישוב, מיקוד "
          value={address}
